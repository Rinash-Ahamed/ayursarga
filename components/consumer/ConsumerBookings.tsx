...
export function ConsumerBookings() {
  ...
  const { items, error: loadError, isLoading, hasMore, loadMore, patchItem } = usePaginatedList<BookingDocument>(loader, "Bookings could not be loaded.");
  const error = actionError ?? loadError;
  async function cancel(item: DocumentRecord<BookingDocument>) { setBusy(item.id); try { await cancelConsumerBooking(item.id, item); patchItem(item.id, { status: "cancelled" }); } catch { setActionError("This booking could not be cancelled."); } finally { setBusy(null); } }
  return <PortalShell role="consumer" title="My bookings"><PortalFeedback error={error} empty={!error && !isLoading && items.length === 0 ? "You have no booking requests yet." : undefined} />
