...
export function ConsumerProfile() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!firebaseUser) return;
    ...
    try { await updateUserProfile(firebaseUser.uid, { name: String(data.get("name")), phone: String(data.get("phone") || "") }); await refreshUserProfile(); setMessage("Your profile has been updated."); }
    catch { setMessage("We could not update your profile. Check the details and try again."); } finally { setBusy(false); } }
