...
type HospitalFormDefaults = Partial<Pick<HospitalDocument,
  "name" | "email" | "phone" | "hospitalPhone1" | "hospitalPhone2" | "city" | "district" | "state" | "address" | "description" | "commissionPercentage"
>>;
...
export function HospitalFormFields({
  ...
  return <>
    ...
    <label>Owner WhatsApp No. *<input name="phone" type="tel" defaultValue={defaultValues.phone} required minLength={7} maxLength={25} aria-invalid={Boolean(errors.phone)} /><FieldError field="phone" errors={errors} /></label>
    <label>Hospital Phone 1 *<input name="hospitalPhone1" type="tel" defaultValue={defaultValues.hospitalPhone1} required minLength={7} maxLength={25} aria-invalid={Boolean(errors.hospitalPhone1)} /><FieldError field="hospitalPhone1" errors={errors} /></label>
    <label>Hospital Phone 2 <small>Optional</small><input name="hospitalPhone2" type="tel" defaultValue={defaultValues.hospitalPhone2 ?? ""} minLength={7} maxLength={25} aria-invalid={Boolean(errors.hospitalPhone2)} /><FieldError field="hospitalPhone2" errors={errors} /></label>
    ...
    <label>District *<select name="district" defaultValue={defaultValues.district ?? ""} required aria-invalid={Boolean(errors.district)}><option value="" disabled>Select district</option>{KERALA_DISTRICTS.map((district) => <option value={district} key={district}>{district}</option>)}</select><FieldError field="district" errors={errors} /></label>
