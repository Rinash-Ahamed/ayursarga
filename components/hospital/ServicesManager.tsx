...
export function ServicesManager() {
  ...
  const { items, error: loadError, isLoading, hasMore, reload, loadMore, patchItem } = usePaginatedList<ServiceDocument>(loader, "We could not load your services. Refresh the page and try again.");
  const error = actionError ?? loadError;
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!id) return; const form = event.currentTarget; const data = new FormData(form); setBusy(true); setActionError(null); setActionMessage(null);
    try { await createService({ hospitalId: id, name: String(data.get("name")), description: String(data.get("description")), price: Number(data.get("price")), durationMinutes: data.get("duration") ? Number(data.get("duration")) : null, status: "active" }); form.reset(); await reload(); setActionMessage("The service has been added."); }
    catch { setActionError("We could not add the service. Check the details and try again."); } finally { setBusy(false); } }
  async function toggle(item: DocumentRecord<ServiceDocument>) { const status = item.status === "active" ? "inactive" : "active"; setBusy(true); setActionError(null); setActionMessage(null); try { await updateService(item.id, { status }, item); patchItem(item.id, { status }); setActionMessage(`The service is now ${status}.`); } catch { setActionError("We could not change this service's status. Refresh the page and try again."); } finally { setBusy(false); } }
  ...
  return <PortalShell role="hospital" title="Services"><PortalLoadGuard loading={isLoading} error={loadError} hasData={items.length > 0} fallbackHref="/hospital" loadingMessage="Loading hospital services…" /><form className="portal-card portal-form" onSubmit={submit} style={{ marginBottom: 26 }}>
    ...
    <PortalFeedback error={error} empty={!error && !isLoading && items.length === 0 ? "No services yet. Complete the form above to add your first service." : undefined} /><PortalToast message={actionMessage} /><div className="portal-list">{items.map((item) => <article className="portal-row" key={item.id}><div><h3>{item.name}</h3><p>{formatCurrency(item.price)} · {item.durationMinutes ? `${item.durationMinutes} minutes` : "Flexible duration"}</p></div><div><span className="portal-status" data-status={item.status}>{item.status}</span><div className="portal-actions"><button className="portal-button secondary" disabled={busy} onClick={() => void edit(item)}>Edit</button><button className="portal-button secondary" disabled={busy} onClick={() => void toggle(item)}>{item.status === "active" ? "Deactivate" : "Activate"}</button></div></div></article>)}</div><PortalPagination hasMore={hasMore} isLoading={isLoading} onLoadMore={() => void loadMore()} />
  </PortalShell>;
}
