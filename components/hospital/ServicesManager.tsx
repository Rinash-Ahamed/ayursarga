...
export function ServicesManager() {
  ...
  async function toggle(item: DocumentRecord<ServiceDocument>) { const status = item.status === "active" ? "inactive" : "active"; setBusy(true); try { await updateService(item.id, { status }, item); patchItem(item.id, { status }); } catch { setActionError("We could not change this service's status. Refresh the page and try again."); } finally { setBusy(false); } }
