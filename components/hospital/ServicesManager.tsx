...
export function ServicesManager() {
  ...
  useEffect(() => { if (!id) return; void listHospitalServices(id, { pageSize: 20 }).then((page) => setItems(page.documents)).catch(() => setError("Services could not be loaded.")); }, [id]);
  ...
  async function edit(item: DocumentRecord<ServiceDocument>) {
    ...
    setBusy(true); try { await updateService(item.id, { name, price: Number(price), description }); await load(); }
    catch { setError("Service could not be updated."); } finally { setBusy(false); }
  }
