...
export function AdminBookings() {
  ...
  return <PortalShell role="admin" title="Bookings">
    ...
    <div className="portal-list">{items.map((item) => <article className="portal-row" key={item.id}>
      <div><h3>{formatCurrency(item.servicePrice)}</h3><p>Estimated commission {formatCurrency(item.estimatedCommission)} · Hospital {selectedHospital?.id === item.hospitalId ? selectedHospital.name : item.hospitalId}</p></div>
