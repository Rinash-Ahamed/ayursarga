...
export function HospitalsManager() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!firebaseUser) return; const form = event.currentTarget; const data = new FormData(form); setBusy(true); setActionError(null);
    try { await createHospital({ name: String(data.get("name")), description: String(data.get("description")), email: String(data.get("email")), phone: String(data.get("phone")), address: String(data.get("address")), city: String(data.get("city")), state: String(data.get("state")), imageUrl: null, status: "pending", isPublic: false, commissionPercentage: Number(data.get("commission")) }, firebaseUser.uid); form.reset(); await reload(); }
    catch { setActionError("Hospital could not be created."); } finally { setBusy(false); } }
  async function change(item: DocumentRecord<HospitalDocument>, changes: Partial<HospitalDocument>) { setBusy(true); try { await updateHospital(item.id, changes, item); patchItem(item.id, changes); } catch { setActionError("Hospital could not be updated."); } finally { setBusy(false); } }
  ...
  return <PortalShell role="admin" title="Hospitals"><form className="portal-card portal-form" onSubmit={submit} style={{ marginBottom: 26 }}>
    ...
    <PortalFeedback error={error} empty={!error && !isLoading && items.length === 0 ? "No hospitals have been created." : undefined} /><div className="portal-list">{items.map((item) => <article className="portal-card" key={item.id}><div style={{ display: "flex", justifyContent: "space-between", gap: 16 }}><h3>{item.name}</h3><span className="portal-status">{item.status} · {item.isPublic ? "public" : "private"}</span></div><p>{item.city}, {item.state} · Commission {item.commissionPercentage}%</p><div className="portal-actions"><button className="portal-button" disabled={busy} onClick={() => void change(item, { status: item.status === "active" ? "inactive" : "active", isPublic: item.status !== "active" })}>{item.status === "active" ? "Deactivate" : "Activate"}</button><button className="portal-button secondary" onClick={() => setCommission(item)}>Set commission</button></div></article>)}</div><PortalPagination hasMore={hasMore} isLoading={isLoading} onLoadMore={() => void loadMore()} />
  </PortalShell>;
}
